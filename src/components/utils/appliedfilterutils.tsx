import { AppliedQueryFilter, FiltersState } from "@yext/search-headless-react";
...
function createGroupedFilters(
  ...
  const groupedFilters: Record<string, DisplayableFilter[]> = mapArrayToObject(
    allFilters,
    getGroupLabel
  );
...
export function getGroupedAppliedFilters(
  ...
  const displayableStaticFilters = getDisplayableStaticFilters(
    appliedFiltersState?.static,
    staticFiltersGroupLabels
  );
  const displayableFacets = getDisplayableAppliedFacets(
    appliedFiltersState?.facets
  );
  ...
  const prunedAppliedFilters = pruneAppliedFilters(
    appliedFilters,
    hiddenFields
  );
  const prunedNlpFilters = pruneNlpFilters(
    displayableNlpFilters,
    prunedAppliedFilters,
    hiddenFields
  );
